...
def require_columns(df: DataFrame, cols: List[str], label: str):
    ...
    if missing:
        raise Exception(
            f"[{label}] Missing required columns: {missing}. Available: {df.columns}"
        )
...
# Args
# -------------------------
# RUN_ID is optional: if not provided, we generate one.
required_args = [
    "JOB_NAME",
    "S3_BUCKET",
    "LANDING_PREFIX",
...
args = getResolvedOptions(
    sys.argv, required_args + ["RUN_ID"] if "--RUN_ID" in sys.argv else required_args
)
...
run_id = (
    args.get("RUN_ID") or f"run_{datetime.datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
)
...
# -------------------------
zones_raw = (
    spark.read.option("header", "true").option("inferSchema", "true").csv(zone_csv_path)
)
...
txns = txns_raw.withColumn(pu_col, F.trim(F.col(pu_col).cast("string"))).withColumn(
    do_col, F.trim(F.col(do_col).cast("string"))
)
...
zones_null_key = zones.filter(
    F.col(zone_key_col).isNull() | (F.length(F.col(zone_key_col)) == 0)
).count()
...
zones_dup_groups = (
    zones.groupBy(zone_key_col).count().filter(F.col("count") > 1).count()
)
...
zones_invalid = zones.filter(
    F.col(zone_key_col).isNull() | (F.length(F.col(zone_key_col)) == 0)
)
zones_valid = zones.filter(
    ~(F.col(zone_key_col).isNull() | (F.length(F.col(zone_key_col)) == 0))
)

zone_keys = zones_valid.select(F.col(zone_key_col).alias("zkey")).dropDuplicates(
    ["zkey"]
)
...
# -------------------------
tx1 = (
    txns.join(zone_keys, txns[pu_col] == zone_keys["zkey"], "left")
    .withColumn("pu_valid", F.col("zkey").isNotNull())
    .drop("zkey")
)

tx2 = (
    tx1.join(zone_keys, tx1[do_col] == zone_keys["zkey"], "left")
...
txns_valid = tx2.filter(F.col("pu_valid") & F.col("do_valid")).drop(
    "pu_valid", "do_valid"
)
txns_invalid = tx2.filter(~(F.col("pu_valid") & F.col("do_valid"))).drop(
    "pu_valid", "do_valid"
)
...
zones_invalid.write.mode("overwrite").parquet(
    quarantine_path.rstrip("/") + "/zones_invalid/"
)
txns_invalid.write.mode("overwrite").parquet(
    quarantine_path.rstrip("/") + "/trips_orphan_location/"
)
...
dq_report = {
    ...
    "inputs": {
        "zones_csv": zone_csv_path,
        "transactions_parquet_prefix": txn_parquet_path,
    },
    "outputs": {
...
if fail_on_dq and not overall_pass:
    raise Exception(
        "QUALITY GATE FAILED. Check dq-reports/ under OUT_PREFIX for details."
    )
